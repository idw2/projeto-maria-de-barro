...
Class Querys extends Model {
    ...
    public function get_qntdd($codproduto) {

        $query = $this->db->query("SELECT QUANTIDADE as QNTDD FROM produtos WHERE CODPRODUTO='{$codproduto}'");
...
$qntdd = $model->get_qntdd($_POST["CODPRODUTO"]);

if ((int) $_POST["QUANTIDADE"] > (int) $qntdd) {
    echo "error";
} else {
